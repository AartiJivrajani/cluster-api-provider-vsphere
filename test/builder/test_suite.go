	...
	/*if s.flags.IntegrationTestsEnabled {
		...
		s.envTest = envtest.Environment{
			CRDDirectoryPaths: []string{
				...
				filepath.Join(FindModuleDir("sigs.k8s.io/cluster-api"), "config", "crd", "bases"),
				filepath.Join(FindModuleDir("sigs.k8s.io/cluster-api"), "bootstrap", "kubeadm", "config", "crd", "bases"),
				filepath.Join(FindModuleDir("sigs.k8s.io/cluster-api"), "controlplane", "kubeadm", "config", "crd", "bases"),
				filepath.Join(FindModuleDir("gitlab.eng.vmware.com/core-build/cluster-api-provider-wcp"), "config", "crd", "bases"),
			},
...
//
// Returns nil if unit testing is disabled.
func (s *TestSuite) NewUnitTestContextForControllerWithTanzuKubernetesCluster(vsphereCluster *vmwarev1.VSphereCluster, prototypeCluster bool, initObjects ...runtime.Object) *UnitTestContextForController {
	if s.flags.UnitTestsEnabled {
