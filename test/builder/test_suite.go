...
import (
	...
	"gitlab.eng.vmware.com/core-build/guest-cluster-controller/lib/wcpfss"
	"gitlab.eng.vmware.com/core-build/guest-cluster-controller/pkg/builder"
	"gitlab.eng.vmware.com/core-build/guest-cluster-controller/pkg/manager"
...
const (
	TestServiceAccountCMNamespace = "test-namespace"
	TestServiceAccountCMName      = "test-cm"
)
...
type TestSuite struct {
	context.Context
	FeatureStates         *wcpfss.WCPFeatureStateConfig
...
// NewTestSuiteForCAPIWebhook returns a new test suite used for unit and
// integration testing validating webhooks created using the "pkg/builder"
// package.
func NewTestSuiteForCAPIWebhook(
	addToManagerFn manager.AddToManagerFunc,
...
func (s *TestSuite) init(addToManagerFn manager.AddToManagerFunc, newReconcilerFn builder.NewReconcilerFunc, additionalAPIServerFlags ...string) {
	...
	featureStates := &wcpfss.WCPFeatureStateConfig{}
	if err := featureStates.Setup(); err != nil {
		panic(errors.Wrap(err, "unable to initialize feature states"))
	}
	...
	if s.flags.IntegrationTestsEnabled {
		...
		s.envTest = envtest.Environment{
			CRDDirectoryPaths: []string{
				...
				filepath.Join(s.flags.RootDir, "test", "stubcrds"),
				filepath.Join(s.flags.RootDir, "test", "stubcrds", "cert-manager"),
				filepath.Join(testutil.FindModuleDir("sigs.k8s.io/cluster-api"), "config", "crd", "bases"),
				filepath.Join(testutil.FindModuleDir("sigs.k8s.io/cluster-api"), "bootstrap", "kubeadm", "config", "crd", "bases"),
				filepath.Join(testutil.FindModuleDir("sigs.k8s.io/cluster-api"), "controlplane", "kubeadm", "config", "crd", "bases"),
...
func (s *TestSuite) NewUnitTestContextForController(initObjects ...runtime.Object) *UnitTestContextForController {
	return s.NewUnitTestContextForControllerWithTanzuKubernetesCluster(nil, false, initObjects...)
}
...
func (s *TestSuite) NewUnitTestContextForControllerWithPrototypeCluster(initObjects ...runtime.Object) *UnitTestContextForController {
	return s.NewUnitTestContextForControllerWithTanzuKubernetesCluster(nil, true, initObjects...)
}

// NewUnitTestContextForControllerWithTanzuKubernetesCluster returns a new unit test context for this
// suite's reconciler initialized with the given tanzukubernetescluster.
//
// Returns nil if unit testing is disabled.
func (s *TestSuite) NewUnitTestContextForControllerWithTanzuKubernetesCluster(tkc *tkgv1.TanzuKubernetesCluster, prototypeCluster bool, initObjects ...runtime.Object) *UnitTestContextForController {
	if s.flags.UnitTestsEnabled {
		ctx := NewUnitTestContextForController(s.newReconcilerFn, tkc, prototypeCluster, initObjects, nil)
...
// NewUnitTestContextForControllerWithGuestClusterObjects returns a new unit test context for this
// suite's reconciler.
//
// Returns nil if unit testing is disabled.
...
//
// Returns nil if unit testing is disabled.
func (s *TestSuite) NewUnitTestContextForValidatingWebhook(initObjects ...runtime.Object) *UnitTestContextForValidatingWebhook {
	if s.flags.UnitTestsEnabled {
		ctx := NewUnitTestContextForValidatingWebhook(s.validator, initObjects...)
		SetTKRRefForVersion(ctx.TanzuKubernetesCluster, FakeDistributionVersion)
		SetTKRRefForVersion(ctx.OldTanzuKubernetesCluster, ctx.TanzuKubernetesCluster.Spec.Distribution.Version)
		return ctx
	}
	return nil
}
...
//
// Returns nil if unit testing is disabled.
func (s *TestSuite) NewUnitTestContextForMutatingWebhook(requiredVirtualMachineImageVersion string, additionalVirtualMachineImageVersions ...string) *UnitTestContextForMutatingWebhook {
	if s.flags.UnitTestsEnabled {
		ctx := NewUnitTestContextForMutatingWebhook(s.mutator, append(additionalVirtualMachineImageVersions, requiredVirtualMachineImageVersion)...)
		SetTKRRefForVersion(ctx.TanzuKubernetesCluster, requiredVirtualMachineImageVersion)
		SetTKRRefForVersion(ctx.OldTanzuKubernetesCluster, requiredVirtualMachineImageVersion)
		return ctx
	}
	return nil
}
...
func (s *TestSuite) postConfigureManager() {
	...
	if s.isWebhookTest() {
		By("installing the webhook(s)", func() {
			...
			// UNMARSHAL the contents of the validating webhook file into MutatingWebhookdConfiguration and
